...
public class NativeAdDemoRender {
    ...
    public static int widget_width = 100;
    public static int widget_height = 100;
...
//        clickableViews.add(mDownloadButton);
        ...
        widgetView = adData.getWidgetView(widget_width,widget_height);
        if (widgetView != null) {
            creativeViewList.add(widgetView);
            int width_px = dipsToPixels(widgetView.getContext(), widget_width);
            int height_px = dipsToPixels(widgetView.getContext(), widget_height);
            if (widgetView.getParent() != null) {
                ViewGroup parent = (ViewGroup) widgetView.getParent();
                parent.removeView(widgetView);
                Log.d("WindSDK", "parent:" + parent);
            }
            FrameLayout.LayoutParams layoutParams = new FrameLayout.LayoutParams(width_px, height_px);
            layoutParams.gravity = Gravity.CENTER;
            ((ViewGroup)nativeAdView).addView(widgetView,layoutParams);
        }
        ...
        if (patternType == NativeAdPatternType.NATIVE_BIG_IMAGE_AD) {
            ...
            if (Constants.native_image_res) {
                List<SigImage> imageList = adData.getImageList();

                for (int i = 0; i < imageList.size(); i++) {
                    SigImage imageItem = imageList.get(i);
                    if (imageViews.size() > i) {
                        ImageView imageView = imageViews.get(i);
                        Glide.with(context).load(imageItem.getImageUrl()).into(imageView);
                        Toast.makeText(context, "原生图片素材 " + imageItem.getImageUrl(), Toast.LENGTH_LONG).show();
                    }
                }

                Toast.makeText(context, "使用原生图片素材展示", Toast.LENGTH_LONG).show();
            } else {
                Toast.makeText(context, "使用原生绑定接口实现图片展示", Toast.LENGTH_LONG).show();
            ...
            if (showWithoutAppInfo) {
                adData.bindMediaViewWithoutAppInfo(mMediaViewLayout, nativeADMediaListener);
            } else {
    ...
    public void setShowWithoutAppInfo(boolean showWithoutAppInfo) {
        this.showWithoutAppInfo = showWithoutAppInfo;
    }
