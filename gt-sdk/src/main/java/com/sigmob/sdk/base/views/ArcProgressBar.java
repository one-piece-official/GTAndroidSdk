...
public class ArcProgressBar extends View {
    ...
    private void init() {
        ...
        arcRect = new RectF(arcCenter.x - radius, arcCenter.y - radius,
                arcCenter.x + radius, arcCenter.y + radius);
        leftArcPoint = new PointF(arcCenter.x - (float) (Math.cos(dltAngle) * (arcRect.width() / 2)),
                arcCenter.y - (float) (Math.sin(dltAngle) * (arcRect.height() / 2)));
        rightArcPoint = new PointF(arcCenter.x + (float) (Math.cos(dltAngle) * (arcRect.width() / 2)),
                arcCenter.y - (float) (Math.sin(dltAngle) * (arcRect.height() / 2)));
    ...
    protected void onDraw(Canvas canvas) {
        ...
        // Draw outer arc
        canvas.drawArc(arcRect, (float) (180 + v),
                v1, false, outPaint);
    ...
    private void drawLeftArrow(Path path, PointF center, PointF arcPoint, float arrowSize) {
        ...
        PointF p1 = new PointF(arcPoint.x + 0.5f * arrowSize * (float) Math.sin(Math.PI / 2 - Math.atan(k)),
                arcPoint.y + 0.5f * arrowSize * (float) Math.cos(Math.PI / 2 - Math.atan(k)));
        PointF p2 = new PointF(arcPoint.x - 0.5f * arrowSize * (float) Math.sin(Math.PI / 2 - Math.atan(k)),
                arcPoint.y - 0.5f * arrowSize * (float) Math.cos(Math.PI / 2 - Math.atan(k)));
        PointF p3 = new PointF(p2.x - arrowSize * (float) Math.sin(Math.PI / 3 - Math.atan(1 / k)),
                p2.y + arrowSize * (float) Math.cos(Math.PI / 3 - Math.atan(1 / k)));
    ...
    private void drawRightArrow(Path path, PointF center, PointF arcPoint, float arrowSize) {
        ...
        PointF p2 = new PointF(arcPoint.x - 0.5f * arrowSize * (float) Math.sin(Math.PI / 2 - Math.atan(k)),
                arcPoint.y - 0.5f * arrowSize * (float) Math.cos(Math.PI / 2 - Math.atan(k)));
        PointF p3 = new PointF(p2.x + arrowSize * (float) Math.sin(Math.PI * 2 / 3 + Math.atan(1 / k)),
                p2.y + arrowSize * (float) Math.cos(Math.PI * 2 / 3 + Math.atan(1 / k)));
