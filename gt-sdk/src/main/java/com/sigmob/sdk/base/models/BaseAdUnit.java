...
import static com.sigmob.sdk.base.common.AdFormat.FULLSCREEN_VIDEO;
import static com.sigmob.sdk.base.common.AdFormat.REWARD_VIDEO;
import static com.sigmob.sdk.base.common.AdFormat.SPLASH;
...
import com.sigmob.sdk.nativead.NativeAdConfig;
import com.sigmob.sdk.newInterstitial.InterstitialConfig;
import com.sigmob.sdk.splash.SplashAdConfig;
    ...
    public static BaseAdUnit adUnit(Ad ad, String request_id, LoadAdRequest adRequest, SlotAdSetting slotAdSetting, Template scene, String uid, Integer expiration_time, BiddingResponse bidding_response) {
        ...
        try {
            ...
            if (materialMeta != null) {
                ...
                if (materialMeta.creative_type == CreativeType.CreativeTypeVideo_Html_Snippet.getCreativeType()
                        || materialMeta.creative_type == CreativeType.CreativeTypeVideo_transparent_html.getCreativeType()) {
...
//                        SigmobFileUtil.writeToBuffer(materialMeta.html_snippet.toByteArray(),baseAdUnit.getEndCardIndexPath());
    ...
    private static void initAdTrackerMap(BaseAdUnit adUnit) {
        List<Tracking> trackings = adUnit.getAd_tracking();
        adUnit.adTrackersMap = new HashMap<>();

        for (Tracking adTracking : trackings) {
            List<SigAdTracker> trackers = createTrackersForUrls(adTracking.tracking_url,
                    adTracking.tracking_event_type,
                    adUnit.request_id,
                    adUnit.getTrackingRetryNum());
    ...
    public static List<SigAdTracker> createTrackersForUrls(final List<String> urls, String event,
                                                           String request_id, Integer retryNum) {
...
//                    return DriftAdConfig.getAdConfig(this);
    ...
    public boolean noHasDownloadDialog() {
        return (getInteractionType() != InterActionType.DownloadType
                && getInteractionType() != InterActionType.DownloadOpenDeepLinkType)
                || getadPrivacy() == null
                || !isDownloadDialog();
    }
    ...
    public boolean isEndCardIndexExist() {
        ...
        if (!TextUtils.isEmpty(getEndcard_url()) && (getCreativeType() == CreativeType.CreativeTypeVideo_Tar.getCreativeType() ||
                getCreativeType() == CreativeType.CreativeTypeVideo_Tar_Companion.getCreativeType())) {
    ...
    public String getVideo_url() {
        ...
        if (ad != null && ad.materials.size() > 0) {
            ...
            if (ad_type == 5) {
                ...
                if (video != null)
                    return StringUtil.getUrl(video.url);
            } else {
                MaterialMeta materialMeta = ad.materials.get(0);
                if (materialMeta != null) {
    ...
    public String getVideo_md5() {
        if (!TextUtils.isEmpty(video_md5))
            return video_md5;
    ...
    public String getEndcard_md5() {
        if (!TextUtils.isEmpty(endcard_md5))
            return endcard_md5;
   ...
   public void enableUseDownloadApk(boolean enable) {
    ...
    public int getButtonColor() {
        ...
        if (material != null && material.button_color != null) {
            return Color.argb((int) (material.button_color.alpha * 255),
                    material.button_color.red, material.button_color.green, material.button_color.blue);
        }
