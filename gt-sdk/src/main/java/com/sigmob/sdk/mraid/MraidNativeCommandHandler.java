...
public class MraidNativeCommandHandler {
    ...
    public void createCalendarEvent(final Context context, final Map<String, String> params)
        ...
        if (isCalendarAvailable(context)) {
            ...
            throw new MraidCommandException("Action is " +
                    "unsupported on this device (need SigmobAndroid version Ice Cream Sandwich or " +
                    "above)");
        }
    }

    public void storePicture(final Context context,
                             final String imageUrl,
                             MraidCommandFailureListener failureListener) throws MraidCommandException {
        if (!isStorePictureSupported(context)) {
            SigmobLog.e("Error downloading file - the device does not have an SD card mounted, or " +
                    "the SigmobAndroid permission is not granted.");
            throw new MraidCommandException("Error downloading file " +
                    " - the device does not have an SD card mounted, " +
                    "or the SigmobAndroid permission is not granted.");
        }

//        if (context instanceof Activity) {
//            showUserDialog(context, imageUrl, failureListener);
//        } else {
//            SigToast.makeText(context, "Downloading image to Picture gallery...", Toast.LENGTH_SHORT).show();
...
//
//        return IntentUtil.deviceCanHandleIntent(context, telIntent);
        return false;
    }
...
//
//        return IntentUtil.deviceCanHandleIntent(context, smsIntent);
        return false;
...
//                && ClientMetadata.isPermissionGranted(context, WRITE_EXTERNAL_STORAGE);
    }
...
//
//        return IntentUtil.deviceCanHandleIntent(context, calendarIntent);
    }
...
//                        SigmobLog.e( "Image successfully saved.");
//                    }
//
//                    @Override
//                    public void onFailure() {
//                        SigToast.makeText(context, "Image failed to download.", Toast.LENGTH_SHORT).show();
//                        SigmobLog.e( "Error downloading and saving image file.");
...
//
//        public DownloadImageAsyncTask( final Context context,
//                 final DownloadImageAsyncTaskListener listener) {
//            super();
...
//                        hh.getHttpUrlConnection(uriString);
...
//
//
//        private String getFileNameForUriAndHeaders( final URI uri,
...
//            SigmobMediaScannerConnectionClient mediaScannerConnectionClient =
//                    new SigmobMediaScannerConnectionClient(filename, null);
...
//    }
    ...
    private static class SigmobMediaScannerConnectionClient
            implements MediaScannerConnection.MediaScannerConnectionClient {
        private final String mFilename;
        private final String mMimeType;
        private MediaScannerConnection mMediaScannerConnection;

        private SigmobMediaScannerConnectionClient(String filename, String mimeType) {
            mFilename = filename;
            mMimeType = mimeType;
        }
