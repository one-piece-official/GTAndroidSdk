...
import static com.sigmob.sdk.downloader.core.Util.ETAG;
import static com.sigmob.sdk.downloader.core.cause.ResumeFailedCause.RESPONSE_CREATED_RANGE_NOT_FROM_0;
import static com.sigmob.sdk.downloader.core.cause.ResumeFailedCause.RESPONSE_ETAG_CHANGED;
import static com.sigmob.sdk.downloader.core.cause.ResumeFailedCause.RESPONSE_PRECONDITION_FAILED;
import static com.sigmob.sdk.downloader.core.cause.ResumeFailedCause.RESPONSE_RESET_RANGE_NOT_FROM_0;
...
import com.sigmob.sdk.downloader.core.exception.NetworkPolicyException;
import com.sigmob.sdk.downloader.core.exception.ResumeFailedException;
import com.sigmob.sdk.downloader.core.exception.ServerCanceledException;
...
public class DownloadStrategy {
    ...
    public boolean inspectAnotherSameInfo(DownloadTask task, BreakpointInfo info,
                                          long instanceLength) {
        ...
        if (anotherInfo.getTotalOffset()
                <= FileDownload.with().downloadStrategy().reuseIdledSameInfoThresholdBytes()) {
            return false;
        }
    ...
    public boolean isUseMultiBlock(final boolean isAcceptRange) {
        ...
        if (!FileDownload.with().outputStreamFactory().supportSeek()) return false;
    ...
    public void inspectFilenameFromResume(String filenameOnStore,
                                          DownloadTask task) {
    ...
    public void validFilenameFromResponse(String responseFileName,
                                          DownloadTask task,
                                          BreakpointInfo info) throws IOException {
    ...
    protected String determineFilename(String responseFileName,
    ...
    public boolean validFilenameFromStore(DownloadTask task) {
        final String filename = FileDownload.with().breakpointStore()
    ...
    public static class ResumeAvailableResponseCheck {
        ...
        public void inspect() throws IOException {
            ...
            final ResumeFailedCause resumeFailedCause = FileDownload.with().downloadStrategy()
            ...
            final boolean isServerCancelled = FileDownload.with().downloadStrategy()
    ...
    public void inspectNetworkAvailable() throws UnknownHostException {
        ...
        if (manager == null) {
            manager = (ConnectivityManager) FileDownload.with().context()
