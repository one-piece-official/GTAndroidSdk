...
import com.sigmob.sdk.downloader.IRedirectHandler;
import com.sigmob.sdk.downloader.RedirectUtil;
    ...
    static final class RedirectHandler implements IRedirectHandler {
        ...
        public void handleRedirect(
            ...
            while (RedirectUtil.isRedirect(responseCode)) {
                ...
                URI locationUri = URI.create(redirectLocation);
                if (locationUri != null && !locationUri.isAbsolute()) {
                    // 如果location是相对URL，将其解析为绝对URL
                    try {
                        locationUri = ((DownloadUrlConnection) originalConnection).url.toURI().resolve(locationUri);
                        redirectLocation = locationUri.toString();
                    } catch (Throwable e) {
