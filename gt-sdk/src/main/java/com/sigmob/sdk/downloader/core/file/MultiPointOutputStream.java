...
import com.sigmob.sdk.downloader.core.exception.PreAllocateException;
...
public class MultiPointOutputStream {
    ...
    private static final ExecutorService FILE_IO_EXECUTOR = new ThreadPoolExecutor(0,
            ...
            Util.threadFactory("FileDownload file io", false));
    ...
    MultiPointOutputStream( final DownloadTask task,
                            BreakpointInfo info,
                            DownloadStore store,
        ...
        this.store = store;
        this.supportSeek = FileDownload.with().outputStreamFactory().supportSeek();
        this.isPreAllocateLength = FileDownload.with().processFileStrategy()
    ...
    public MultiPointOutputStream( DownloadTask task,
                                   BreakpointInfo info,
                                   DownloadStore store) {
    ...
    public synchronized void cancel() {
        ...
        try {
            ...
            if (syncFuture != null && !syncFuture.isDone()) {
                inspectValidPath();
                FileDownload.with().processFileStrategy().getFileLock().increaseLock(path);
                try {
                    ensureSync(true, -1);
                } finally {
                    FileDownload.with().processFileStrategy().getFileLock().decreaseLock(path);
                }
            }
        } finally {
    ...
    synchronized DownloadOutputStream outputStream(int blockIndex) throws IOException {
        ...
        if (outputStream == null) {
            ...
            outputStream = FileDownload.with().outputStreamFactory().create(
                    FileDownload.with().context(),
                    uri,
                    flushBufferSize);
            if (supportSeek) {
