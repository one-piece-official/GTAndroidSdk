...
import static com.sigmob.sdk.downloader.core.Util.ACCEPT_RANGES;
import static com.sigmob.sdk.downloader.core.Util.CHUNKED_CONTENT_LENGTH;
import static com.sigmob.sdk.downloader.core.Util.CONTENT_DISPOSITION;
...
import static com.sigmob.sdk.downloader.core.Util.IF_MATCH;
import static com.sigmob.sdk.downloader.core.Util.METHOD_HEAD;
import static com.sigmob.sdk.downloader.core.Util.RANGE;
import static com.sigmob.sdk.downloader.core.Util.TRANSFER_ENCODING;
import static com.sigmob.sdk.downloader.core.Util.VALUE_CHUNKED;
...
import com.sigmob.sdk.downloader.core.exception.DownloadSecurityException;
...
public class ConnectTrial {
    ...
    public void executeTrial() throws IOException {
        FileDownload.with().downloadStrategy().inspectNetworkOnWifi(task);
        FileDownload.with().downloadStrategy().inspectNetworkAvailable();

        DownloadConnection connection = FileDownload.with().connectionFactory().create(task.getUrl());
        ...
        try {
            ...
            final DownloadListener listener = FileDownload.with().callbackDispatcher().dispatch();
    ...
    boolean isNeedTrialHeadMethodForInstanceLength(
            long oldInstanceLength, DownloadConnection.Connected connected) {
    ...
    void trialHeadMethodForInstanceLength() throws IOException {
        final DownloadConnection connection = FileDownload.with().connectionFactory()
                .create(task.getUrl());
        final DownloadListener listener = FileDownload.with().callbackDispatcher().dispatch();
        try {
