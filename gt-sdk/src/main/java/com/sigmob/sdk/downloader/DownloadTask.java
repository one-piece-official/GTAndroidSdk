 ...
 * {@link FileDownload#breakpointStore()} with {@link #getId()}, and it's very welcome to use
...
public class DownloadTask extends IdentifiedTask implements Comparable<DownloadTask> {
    ...
    private volatile long lastTimeOps;
    ...
    public DownloadTask(String url, Uri uri, int priority, int readBufferSize, int flushBufferSize,
                        ...
                        Map<String, List<String>> headerMapFields, String filename,
        ...
        this.id = FileDownload.with().breakpointStore().findOrCreateId(this);
    }
     ...
     * and FileDownload isn't get response yet.
     */
    public String getFilename() {
        return filenameHolder.get()+".tmp";
    }
    ...
    public File getTempFile() {
        final String filename = filenameHolder.get()+".tmp";
        if (filename == null) return null;
        if (tempTargetFile == null) tempTargetFile = new File(directoryFile, filename);
    ...
    public BreakpointInfo getInfo() {
        if (info == null) info = FileDownload.with().breakpointStore().get(id);
        return info;
    }
    ...
    public static void enqueue(DownloadTask[] tasks, DownloadListener listener) {
        ...
        FileDownload.with().downloadDispatcher().enqueue(tasks);
    }
    ...
    public void enqueue(DownloadListener listener) {
        this.listener = listener;
        FileDownload.with().downloadDispatcher().enqueue(this);
    }

    public void enqueue() {
        if (System.currentTimeMillis()-lastTimeOps<500){
            SigToast.makeText(SDKContext.getApplicationContext(),"操作太频繁", Toast.LENGTH_SHORT).show();
            return;
        }
        lastTimeOps = System.currentTimeMillis();
        FileDownload.with().downloadDispatcher().enqueue(this);
    }
    ...
    public void execute(DownloadListener listener) {
        this.listener = listener;
        FileDownload.with().downloadDispatcher().execute(this);
    }
    ...
    public void cancel() {
        ...
        lastTimeOps = System.currentTimeMillis();
        FileDownload.with().downloadDispatcher().cancel(this);
    }
    ...
    public static void cancel(DownloadTask[] tasks) {
        FileDownload.with().downloadDispatcher().cancel(tasks);
    }
    ...
    public static class Builder {
        ...
        public static final int DEFAULT_MIN_INTERVAL_MILLIS_CALLBACK_PROCESS = 1000/* millis **/;
    ...
    public static class TaskHideWrapper {
        ...
        public static void setBreakpointInfo(DownloadTask task,
                                             BreakpointInfo info) {
