...
public class Listener4Assist<T extends Listener4Assist.Listener4Model> implements ListenerAssist {
    ...
    public interface AssistExtend {
        boolean dispatchInfoReady(DownloadTask task,  BreakpointInfo info,
                                  boolean fromBreakpoint,
                                   Listener4Model model);

        boolean dispatchFetchProgress( DownloadTask task, int blockIndex,
                                      long increaseBytes,
                                       Listener4Model model);

        boolean dispatchBlockEnd(DownloadTask task, int blockIndex, Listener4Model model);

        boolean dispatchTaskEnd(DownloadTask task, EndCause cause, Exception realCause,
                                 Listener4Model model);
    }

    public interface Listener4Callback {

        void infoReady(DownloadTask task,  BreakpointInfo info, boolean fromBreakpoint,
                        Listener4Model model);
