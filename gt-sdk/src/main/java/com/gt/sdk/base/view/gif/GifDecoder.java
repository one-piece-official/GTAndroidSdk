...
class GifDecoder {
    ...
    synchronized Bitmap getNextFrame() {
        if (header.frameCount <= 0 || framePointer < 0) {
            ...
            SigmobLog.d("unable to decode frame, frameCount=" + header.frameCount + " framePointer=" + framePointer);
            //}
            status = STATUS_FORMAT_ERROR;
        }
        if (status == STATUS_FORMAT_ERROR || status == STATUS_OPEN_ERROR) {
            //if (Log.isLoggable(TAG, Log.DEBUG)) {
            SigmobLog.d("Unable to decode frame, status=" + status);
            //}
            return null;
        }
        status = STATUS_OK;
        ...
        if (act == null) {
            //if (Log.isLoggable(TAG, Log.DEBUG)) {
            SigmobLog.d("No Valid Color Table for frame #" + framePointer);
            //}
    ...
    int read(InputStream is, int contentLength) {
        if (is != null) {
            try {
                ...
                SigmobLog.d("Error reading data from stream", e);
            }
        } else {
        ...
        try {
            ...
            SigmobLog.d("Error closing stream", e);
        }
    ...
    private int readBlock() {
        ...
        if (blockSize > 0) {
            try {
                ...
                SigmobLog.d("Error Reading Block", e);
                status = STATUS_FORMAT_ERROR;
            }
        }
        return blockSize;
        ...
        if (previousFrame != null && previousFrame.dispose > DISPOSAL_UNSPECIFIED) {
            ...
            if (previousFrame.dispose == DISPOSAL_BACKGROUND) {
                ...
                if (previousImage == null) {
                    ...
                    int topLeft = downsampledIY * downsampledWidth + downsampledIX;
                    previousImage.getPixels(dest, topLeft, downsampledWidth, downsampledIX, downsampledIY, downsampledIW, downsampledIH);
                }
            }
        }
