...
publishaliyun(){
 ...
 # 通知中控平台
          ...
          # android
          curl -s --data-urlencode "os=2" --data-urlencode "version=${VERSION}" --data-urlencode "region=${LOCATION}" --data-urlencode "cdn=${CDN_URL}" --data-urlencode "source=sigmob" ${SYNC_URL}
          if [[ ${CON_ENV} == "manager" ]]; then
              curl -H "Content-Type: application/json" -X POST -d "{\"events\":[{\"version\":\"${VERSION}\",\"region\":\"${LOCATION}\",\"os\":\"Sigmob Android\",\"url\":\"${CDN_URL}\"}]}" "https://www.feishu.cn/flow/api/trigger-webhook/f644af83e65eaea2018db46d83b2a917"
          fi
