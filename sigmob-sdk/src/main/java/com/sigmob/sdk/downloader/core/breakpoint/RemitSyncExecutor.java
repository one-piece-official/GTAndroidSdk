...
public class RemitSyncExecutor implements Handler.Callback {
    ...
    RemitSyncExecutor(RemitAgent agent) {
        ...
        final HandlerThread thread = new HandlerThread("FileDownload RemitHandoverToDB");
        thread.start();

        handler = new Handler(thread.getLooper(), this);
    }

    RemitSyncExecutor(RemitAgent agent, Handler handler,
                      Set<Integer> freeToDBIdList) {
        this.agent = agent;
