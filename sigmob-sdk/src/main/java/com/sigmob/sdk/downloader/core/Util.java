...
import com.sigmob.sdk.downloader.core.breakpoint.BreakpointStoreOnCache;
...
import com.sigmob.sdk.downloader.core.connection.DownloadOkHttp3Connection;
import com.sigmob.sdk.downloader.core.connection.DownloadUrlConnection;
...
public class Util {
    ...
    /**
     * Enable logger used for FileDownload, and print each log with {@link Log}.
     */
    ...
    /**
     * Set the logger which using on FileDownload.
    ...
    public static DownloadStore createDefaultDatabase(Context context) {
        // You can import through com.sigmob.sdk.downloader:sqlite:{version}
        final String storeOnSqliteClassName
                = "com.sigmob.sdk.downloader.core.breakpoint.BreakpointStoreOnSQLite";
    ...
    public static DownloadConnection.Factory createDefaultConnectionFactory() {
        final String okhttpConnectionClassName
                = DownloadOkHttp3Connection.Factory.class.getName();
        try {
    ...
    public static void assembleBlock(DownloadTask task, BreakpointInfo info,
                                     long instanceLength,
                                     boolean isAcceptRange) {
        final int blockCount;
        if (FileDownload.with().downloadStrategy().isUseMultiBlock(isAcceptRange)) {
            blockCount = FileDownload.with().downloadStrategy()
    ...
    public static boolean checkPermission(String permission) {
        final int perm = FileDownload.with().context().checkCallingOrSelfPermission(permission);
    ...
    public static String getFilenameFromContentUri(Uri contentUri) {
        final ContentResolver resolver = FileDownload.with().context().getContentResolver();
    ...
    public static void inspectUserHeader(Map<String, List<String>> headerField)
            throws IOException {
    ...
    public static void addUserRequestHeaderField(Map<String, List<String>> userHeaderField,
    ...
    public static void addDefaultUserAgent(final DownloadConnection connection) {
