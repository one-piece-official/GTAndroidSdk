...
import com.sigmob.sdk.downloader.core.IdentifiedTask;
...
import com.sigmob.sdk.downloader.core.download.DownloadCall;
...
public class DownloadDispatcher {
    ...
    synchronized ExecutorService getExecutorService() {
        if (executorService == null) {
            executorService = new ThreadPoolExecutor(0, Integer.MAX_VALUE,
                    ...
                    Util.threadFactory("FileDownload Download", false));
        }
    ...
    private synchronized void enqueueLocked(DownloadTask[] tasks) {
        ...
        try {
            ...
            FileDownload.with().callbackDispatcher()
            ...
            FileDownload.with().callbackDispatcher().endTasksWithError(errorList, e);
        }
    ...
    private synchronized void filterCanceledCalls(IdentifiedTask task,
        ...
        for (Iterator<DownloadCall> i = readyAsyncCalls.iterator(); i.hasNext(); ) {
            ...
            if (call.task == task || call.task.getId() == task.getId()) {
                if (call.isCanceled() || call.isFinishing()) continue;
...
//                return;
    ...
    private synchronized void handleCanceledCalls(List<DownloadCall> needCallbackCalls,
                                                  List<DownloadCall> needCancelCalls) {
        ...
        if (!needCallbackCalls.isEmpty()) {
            if (needCallbackCalls.size() <= 1) {
                ...
                FileDownload.with().callbackDispatcher().dispatch().taskEnd(call.task,
                        EndCause.CANCELED,
                        null);
            } else {
                ...
                FileDownload.with().callbackDispatcher().endTasksWithCanceled(callbackCanceledTasks);
            }
        }
    }
    ...
    public synchronized boolean isCanceled(DownloadTask task) {
        Util.d(TAG, "isCanceled: " + task.getId());
        for (DownloadCall call : runningSyncCalls) {
            if (call.isCanceled() && call.equalsTask(task)) return true;
        }
    ...
    private boolean inspectForConflict(DownloadTask task,
                                       Collection<DownloadTask> sameTaskList,
                                       Collection<DownloadTask> fileBusyList) {
    ...
    boolean inspectCompleted(DownloadTask task,
                             Collection<DownloadTask> completedCollection) {
        if (task.isPassIfAlreadyCompleted() && StatusUtil.isCompleted(task)) {
            if (task.getFilename() == null && !FileDownload.with().downloadStrategy()
                    .validFilenameFromStore(task)) {
                return false;
            }

            FileDownload.with().downloadStrategy().validInfoOnCompleted(task, store);
    ...
    boolean inspectForConflict(DownloadTask task,
                               Collection<DownloadCall> calls,
                               Collection<DownloadTask> sameTaskList,
                               Collection<DownloadTask> fileBusyList) {
        final CallbackDispatcher callbackDispatcher = FileDownload.with().callbackDispatcher();
    ...
    private synchronized void processCalls() {
        ...
        for (Iterator<DownloadCall> i = readyAsyncCalls.iterator(); i.hasNext(); ) {
            ...
            if (isFileConflictAfterRun(task)) {
                FileDownload.with().callbackDispatcher().dispatch().taskEnd(task, EndCause.FILE_BUSY,
                        null);
                continue;
            }
    ...
    public static void setMaxParallelRunningCount(int maxParallelRunningCount) {
        DownloadDispatcher dispatcher = FileDownload.with().downloadDispatcher();
