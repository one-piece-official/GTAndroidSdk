...
//            img_logo.setVisibility(View.GONE);
        ...
        if (adData.getNetworkId() == 16) {
            FrameLayout.LayoutParams adLogoParams = new FrameLayout.LayoutParams(dpToPx(context, 46), dpToPx(context, 14));
            adLogoParams.gravity = Gravity.END | Gravity.BOTTOM;
            adLogoParams.rightMargin = dpToPx(context, 10);
            adLogoParams.bottomMargin = dpToPx(context, 10);
...
//                    .groupImage1Id(mImagePoster.getId())
//                    .groupImage2Id(mImagePoster.getId())
//                    .groupImage3Id(mImagePoster.getId())
//                    .build();
...
//        }
        ...
        if (imageList != null && imageList.size() > 0) {
            for (int i = 0; i < imageList.size(); i++) {
                ...
                if (wmImage != null) {
                    Log.d("lance", "imageUrlList:" + adData.getNetworkId() + ":" + wmImage.getWidth() + ":" + wmImage.getHeight() + ":" + wmImage.getImageUrl());
                }
            }
        } else {
            Log.d("lance", "imageUrlList is null or size is 0");
        }
